...
function deleteVendorLocation(vendorLocID) {
  let link = '/delete-vendor-location-ajax/';
  let data = {
    id: vendorLocID
  };
...
function deleteRow(vendorLocID){

    let table = document.getElementById("vendor-location-table");
    for (let i = 0, row; row = table.rows[i]; i++) {
       //iterate through rows
       //rows would be accessed using the "row" variable assigned in the for loop
       if (table.rows[i].getAttribute("data-value") == vendorLocID) {
            table.deleteRow(i);
            break;
       }
    }
