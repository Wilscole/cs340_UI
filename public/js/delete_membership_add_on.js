...
function deleteAddOn(addId) {
  console.log(addId)
  let link = '/delete-add-on-ajax/';
  let data = {
      add_on_id: addId
  };
...
function deleteRow(addId) {

  let table = document.getElementById("membership-add-on-table");
  for (let i = 0, row; row = table.rows[i]; i++) {
      //iterate through rows
      //rows would be accessed using the "row" variable assigned in the for loop
      if (table.rows[i].getAttribute("data-value") == addId) {
          table.deleteRow(i);
          break;
      }
  }
